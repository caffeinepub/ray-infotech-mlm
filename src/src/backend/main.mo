...
actor {
  ...
  public type TradingUser = {
    ...
    email : Text;
    phone : Text;
    passwordHash : Text; // store password as-is for demo
    aadhaar : Text;
    pan : Text;
    digilockerRef : Text;
    paymentProof : Text; // base64 image
    selfie : Text;       // base64 image (empty if not provided)
    kycStatus : Text;    // "pending" | "approved" | "rejected"
    paymentStatus : Text; // "pending" | "approved" | "rejected"
    accountStatus : Text; // "active" | "debarred"
    virtualBalance : Nat;
    watchlist : [Text];
    createdAt : Int;     // timestamp
    referredBy : Text;   // member id or empty
    referralBonus : Nat;
    esignature : Text;   // base64 signature or empty
    tcSigned : Bool;
  };
  ...
  public shared func registerTradingUser(user : TradingUser) : async { ok : Bool; message : Text } {
    ...
    if (tradingUsers.containsKey(user.email)) {
      return { ok = false; message = "Email already registered" };
    };
    tradingUsers.add(user.email, user);
    tradingUserCount += 1;
    { ok = true; message = "Registration successful" };
  };

  public query func getTradingUserByEmail(email : Text) : async ?TradingUser {
    tradingUsers.get(email);
  };

  public query func getAllTradingUsers() : async [TradingUser] {
    tradingUsers.values().toArray();
  };

  public shared func updateTradingUser(user : TradingUser) : async { ok : Bool } {
    if (not tradingUsers.containsKey(user.email)) {
      return { ok = false };
    };
    tradingUsers.add(user.email, user);
    { ok = true };
  ...
  public shared func updateTradingUserById(id : Text, kycStatus : Text, paymentStatus : Text, accountStatus : Text, virtualBalance : Nat) : async { ok : Bool } {
    var found = false;
    for ((email, u) in tradingUsers.entries()) {
      if (u.id == id) {
        let updated : TradingUser = {
          u with
          kycStatus;
          paymentStatus;
          accountStatus;
        ...
        tradingUsers.add(email, updated);
        found := true;
      };
    };
    { ok = found };
  ...
  public shared func creditTradingReferralBonus(referrerId : Text) : async { ok : Bool } {
    // Find user by id
    for ((email, u) in tradingUsers.entries()) {
      if (u.id == referrerId) {
        let updated : TradingUser = {
          u with
          virtualBalance = u.virtualBalance + 5;
          referralBonus = u.referralBonus + 5;
        };
        tradingUsers.add(email, updated);
        return { ok = true };
      };
  ...
  public query func nextTradingMemberId() : async Text {
    let num = tradingUserCount + 1;
    let numText = Nat.toText(num);
    let padded = switch (numText.size()) {
      case 1 { "00000" # numText };
      case 2 { "0000" # numText };
      case 3 { "000" # numText };
      case 4 { "00" # numText };
      case 5 { "0" # numText };
