...
export default function PlatformStatistics() {
  ...
  const stats = useMemo(() => {
    ...
    const refundEligible = allMembers.filter(
      (m) => m.directDownlines.length >= 3,
    ).length;
    ...
    for (const member of allMembers) {
      ...
      for (const id of member.directDownlines) {
      ...
      while (queue.length > 0) {
        ...
        if (item.level < 9) {
          for (const childId of downlineMember.directDownlines) {
