    ...
    <div className="flex flex-col items-center">
      ...
      >
        ...
        >
          {isActive ? (
            <UserCheck
              className={`w-4 h-4 ${isRoot ? "text-gold-400" : "text-foreground"}`}
            />
          ) : (
    ...
    <div className="flex justify-center gap-2 sm:gap-4">
      {nodes.map((node) => (
        <div
          key={
            node.member
              ? String(node.member.id)
              : `empty-${node.level}-${node.position}`
          }
              ...
              <div className="flex gap-2 sm:gap-4">
                {node.children.map((child, childIdx) => (
                  <div
                    key={
                      child.member
                        ? String(child.member.id)
                        : `empty-child-${childIdx}`
                    }
                    className="flex flex-col items-center"
                  >
                    <div className="w-px h-4 bg-border" />
    ...
    <div className="bg-card border border-border rounded-2xl p-6">
      ...
      <div className="overflow-x-auto pb-4">
        <div className="min-w-max mx-auto">
          <TreeLevel nodes={[rootNode]} isRoot />
        </div>
      </div>
      ...
      <div className="mt-4 pt-4 border-t border-border grid grid-cols-3 gap-4 text-center">
        ...
        <div>
          <p className="text-2xl font-bold text-foreground font-poppins">
            {3 - member.directDownlines.length > 0
              ? 3 - member.directDownlines.length
              : 0}
          </p>
        ...
        <div>
          <p className="text-2xl font-bold text-gold-400 font-poppins">
            {member.directDownlines.length >= 3
              ? "✓"
              : `${Math.round((member.directDownlines.length / 3) * 100)}%`}
          </p>
