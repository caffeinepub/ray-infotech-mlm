...
// ── Candlestick-style data generation ──────────────────────────────────────────
    ...
    <div className="flex-1 min-w-0">
      ...
      {levels.map((level, i) => (
        <div
          ...
          data-ocid={`equity.${side}.item.${i + 1}`}
        >
          {/* Depth bar */}
          <div
            className={`absolute inset-y-0 ${
              isBid ? "right-0" : "left-0"
            } opacity-10 ${isBid ? "bg-green-400" : "bg-red-400"}`}
...
function NewsCard({ item, idx }: { item: NewsItem; idx: number }) {
  return (
    <div
      ...
      data-ocid={`equity.news.item.${idx + 1}`}
    >
...
export default function EquityDetailPage() {
  const params = useParams({ from: "/equity/$symbol" });
  ...
  const [candles, setCandles] = useState<CandleData[]>(() =>
    generateInitialCandles(asset?.basePrice || 1000),
  );
  const [orderBook, setOrderBook] = useState<OrderBook>(() =>
    generateOrderBook(symbol, asset?.basePrice || 1000),
  );
  ...
  const [volume] = useState(() => Math.floor(Math.random() * 5000000) + 500000);
    ...
    return (
      <div className="max-w-4xl mx-auto px-4 py-12 text-center">
        <p className="text-muted-foreground">
          Symbol &ldquo;{symbol}&rdquo; not found.
        </p>
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-6 pb-28 lg:pb-6">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex flex-wrap items-center gap-2 mb-1">
            ...
            <div
              ...
              data-ocid="equity.market_status.panel"
            >
              <div
      ...
      <div
        ...
        data-ocid="equity.stats.panel"
      >
        {[
          { label: "Today's Open", value: fmt(dayOpen) },
          {
            label: "Day High",
            value: fmt(dayHigh),
      ...
      {/* ── Live Price Chart ───────────────────────────────────────────────── */}
      <Card className="mb-6" data-ocid="equity.chart.panel">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-semibold">
              Live Price Chart (1-min)
            </CardTitle>
      ...
      {/* ── TradingView Widget ─────────────────────────────────────────────── */}
      <Card className="mb-6" data-ocid="equity.tradingview.panel">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
            <CardTitle className="text-sm font-semibold">
        ...
        <CardContent className="p-0 overflow-hidden rounded-b-xl">
          <iframe
            ...
            src={`https://s.tradingview.com/widgetembed/?symbol=${tvSymbol}&interval=5&theme=dark&style=1&locale=en&hide_top_toolbar=0&hide_side_toolbar=0&allow_symbol_change=0`}
            style={{
              width: "100%",
              height: 400,
              border: "none",
      ...
      {/* ── Order Book ────────────────────────────────────────────────────── */}
      <Card className="mb-6" data-ocid="equity.orderbook.panel">
        <CardHeader className="pb-2">
          <div className="flex items-center justify-between">
      ...
      {/* ── News ──────────────────────────────────────────────────────────── */}
      <div className="mb-6" data-ocid="equity.news.panel">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-50 bg-background/95 backdrop-blur-sm border-t border-border px-4 py-3 flex gap-3">
        <Button
          data-ocid="equity.buy.primary_button"
        ...
        <Button
          data-ocid="equity.sell.primary_button"
