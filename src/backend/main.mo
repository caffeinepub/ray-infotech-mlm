...
actor {
  ...
  public type TradingUserResult = { ok : Bool; message : Text };

  stable var tradingUsers = Map.empty<Text, TradingUser>();
  stable var tradingMemberCounter : Nat = 1;

  public shared func registerTradingUser(user : TradingUser) : async TradingUserResult {
  ...
  public query func getTradingUserByEmail(email : Text) : async ?TradingUser {
    var found : ?TradingUser = null;
    for (user in tradingUsers.values()) {
      if (user.email == email) {
        found := ?user;
      };
    };
  ...
  public query func nextTradingMemberId() : async Text {
    let count = tradingUsers.size() + 1;
    let numStr = count.toText();
    let padded = switch (numStr.size()) {
      case (1) { "00000" # numStr };
      case (2) { "0000" # numStr };
      case (3) { "000" # numStr };
      case (4) { "00" # numStr };
  ...
  public shared func creditTradingReferralBonus(referrerId : Text) : async TradingUserResult {
    switch (tradingUsers.get(referrerId)) {
      case (null) { { ok = false; message = "Referrer not found" } };
      case (?referrer) {
        let updated = {
          referrer with
          virtualBalance = referrer.virtualBalance + 5;
          referralBonus = referrer.referralBonus + 5;
        };
        tradingUsers.add(referrerId, updated);
        { ok = true; message = "" };
      };
    };
  };
